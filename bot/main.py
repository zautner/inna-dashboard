...
async def run_logged_command(update: Update, context: Any, command_name: str, action: Callable[[], Awaitable[dict[str, Any]]]):
    ...
    try:
        ...
        if update.message:
            await update.message.reply_text("❌ קרתה שגיאה בזמן ביצוע הפקודה. אפשר לנסות שוב בעוד רגע.")
...
async def status_command(update: Update, context: Any):  # type: ignore[misc]
    ...
    async def action():
        ...
        await update.message.reply_text(f"מצב התור כרגע:\n\n{counts_text}")
        return {
...
async def queue_command(update: Update, context: Any):  # type: ignore[misc]
    ...
    async def action():
        ...
        if not items:
            await update.message.reply_text("אין כרגע פריטים פעילים בתור.")
            return {
                "outcome": "noop",
        ...
        await update.message.reply_text(f"הפריטים הפעילים הקרובים בתור:\n\n{queue_text}")
        return {
...
async def publishing_command(update: Update, context: Any):  # type: ignore[misc]
    ...
    async def action():
        ...
        await update.message.reply_text(f"מצב הפרסום המתוזמן:\n\n{counts_text}\n\n{preview_text}")
        return {
...
async def errors_command(update: Update, context: Any):  # type: ignore[misc]
    ...
    async def action():
        ...
        if not errors:
            await update.message.reply_text("אין כרגע שגיאות קשות אחרונות. ✅")
            return {
                "outcome": "noop",
        ...
        await update.message.reply_text("שגיאות קשות אחרונות:\n\n" + "\n".join(error_lines))
        return {
